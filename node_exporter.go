...
func (h *handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	filters := r.URL.Query()["collect[]"] //prometheus 里的配置，优先级最高

	//修改，将prometheus从consul读取的rdsId下发
	if len(r.URL.Query()["target"]) != 0 {
		collector.RdsList = r.URL.Query()["target"]
		fmt.Println(collector.RdsList)
	} else {
		collector.RdsList = nil
	}
